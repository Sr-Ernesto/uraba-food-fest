// src/lib/db.ts - Lazy initialization to avoid build-time DB access
...
function getDb() {
  ...
  const count = _db.prepare('SELECT COUNT(*) as count FROM restaurants').get() as { count: number };
  if (count.count === 0) {
    ...
    const insertMany = _db.transaction((items: typeof restaurants) => {
...
const db = new Proxy({} as ReturnType<typeof import('better-sqlite3')>, {
  get(_target, prop) {
